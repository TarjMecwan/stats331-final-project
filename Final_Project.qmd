...
variance_table |>
  kable("html", caption = "Variance Components in Regression Model") |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)

```
After assessing our model, we can see that from our $R^2$ = 0.0407, that our model only explains about 4.07% of the variance in poverty rate. We can also see that our residual variance is very high, which tells us that much of the variability in poverty rate is left unexplained by inflation. We can conclude that, although there may be some relationship between inflation and poverty rate, inflation alone is not a strong predictor of poverty rate. 

Though this may seem discouraging, this is still helpful in our analysis. It is important to note here that we are simply exploring the relationship between inflation rate and poverty rate. Inflation being a poor predictor of poverty rate is still helpful in our analysis!  
