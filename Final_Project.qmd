...
To ensure accurate regression results, we: 1. **Filtered out missing values** (`NA`) from both inflation and poverty rates. 2. **Ensured no infinite values** existed after transformation. 3. **Averaged data over all years for each country**, creating a dataset where each country is a single observation.\
- This helps reduce yearly fluctuations and provides a clearer long-term trend.
...
regression_results <- tibble::tibble(
  Term = c("Intercept (β0)", "Slope (β1)", "R-squared"),
  Value = c(intercept, slope, r_squared)
)
knitr::kable(regression_results, digits = 4, caption = "Linear Regression Summary")
```
